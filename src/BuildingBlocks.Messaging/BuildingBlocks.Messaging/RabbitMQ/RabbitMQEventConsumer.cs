    ...
    public class RabbitMQEventConsumer : BackgroundService
    {
        ...
        private async Task ProcessMessage(Type eventType, BasicDeliverEventArgs eventArgs)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Channel is not open, skipping message processing");
                    return;
                }
            ...
            {
                _logger.LogError(ex, "Channel was disposed during message processing - attempting to reinitialize");

                try
                {
                    await InitializeRabbitMQAsync();
                    await SubscribeToEvents();
                    _logger.LogInformation("Successfully reinitialized RabbitMQ connection and subscriptions");
                }
                catch (Exception reinitEx)
                {
                    _logger.LogError(reinitEx, "Failed to reinitialize RabbitMQ connection");
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogError(nackEx, "Failed to nack message after processing error");
                }
            }
        }
        ...
        public override void Dispose()
        {
            try
            {
                _channel?.Dispose();
                _connection?.Dispose(); // فقط Dispose برای IConnection هم
                _logger.LogInformation("RabbitMQ Event Consumer disposed successfully");
            }
            catch (Exception ex)
            {
