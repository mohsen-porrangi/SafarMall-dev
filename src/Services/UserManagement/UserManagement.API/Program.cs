...
namespace UserManagement.API
{
    public class Program
    {
        ...
        private static void ConfigurePipeline(WebApplication app)
        {
            ...
            {
                app.UseSwaggerExtention("User Management API", "v1");
            }

            #region EventHandler Test
            app.MapGet("/ping", () => Results.Ok("pong"));
            app.MapPost("/test-event", async (IMessageBus messageBus, CancellationToken cancellationToken) =>
            {
                var userId = Guid.NewGuid();
                var mobile = "09123456789";

                await messageBus.PublishAsync(new UserActivatedEvent(userId, mobile), cancellationToken);

                return Results.Ok("UserActivatedEvent منتشر شد.");
            });
