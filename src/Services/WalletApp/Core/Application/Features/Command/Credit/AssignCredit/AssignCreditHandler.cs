...
/// </summary>
public class AssignCreditHandler : ICommandHandler<AssignCreditCommand, AssignCreditResponse>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<AssignCreditHandler> _logger;

    public AssignCreditHandler(
        IUnitOfWork unitOfWork,
        ILogger<AssignCreditHandler> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Invalid credit assignment request for user: {UserId}", request.UserId);
            return new AssignCreditResponse
            {
                IsSuccess = false,
        ...
        {
            _logger.LogWarning(ex, "Business rule violation for user: {UserId}", request.UserId);
            return new AssignCreditResponse
            {
                IsSuccess = false,
                ErrorMessage = ex.Message
        ...
        {
            _logger.LogError(ex, "Error assigning credit to user: {UserId}", request.UserId);
            return new AssignCreditResponse
            {
                IsSuccess = false,
