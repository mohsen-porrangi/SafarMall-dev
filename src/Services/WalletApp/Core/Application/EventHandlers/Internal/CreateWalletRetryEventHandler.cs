...
public class CreateWalletRetryEventHandler(
    ...
    ) : IIntegrationEventHandler<CreateWalletRetryEvent>
{  
    public async Task HandleAsync(CreateWalletRetryEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Double-check که واقعاً wallet نداره
            var hasWallet = await unitOfWork.Wallets.UserHasWalletAsync(@event.UserId, cancellationToken);
