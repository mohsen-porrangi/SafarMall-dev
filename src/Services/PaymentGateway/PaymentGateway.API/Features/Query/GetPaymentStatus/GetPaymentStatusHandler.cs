        ...
        ILogger<GetPaymentStatusHandler> logger) : IQueryHandler<GetPaymentStatusQuery, GetPaymentStatusResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            payment.Status = Models.PaymentStatus.Expired;
        ...
        try
        {
            ...
            if (statusResult.IsSuccessful)
            {
                ...
                if (statusResult.Status == Models.PaymentStatus.Paid)
                {
                    payment.MarkAsPaid(
                        statusResult.TransactionId ?? payment.GatewayReference,
                        statusResult.TrackingCode);
                }
                else if (statusResult.Status == Models.PaymentStatus.Failed)
                {
    ...
    private static GetPaymentStatusResponse CreateResponse(Models.Payment payment)
    {
