...
public class CompleteOrderHandler(
    ...
    ILogger<CompleteOrderHandler> logger) : ICommandHandler<CompleteOrderCommand>
{
    public async Task<Unit> Handle(CompleteOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        if (order.LastStatus != OrderStatus.Processing &&
            order.LastStatus != OrderStatus.Pending)
            throw new BadRequestException($"سفارش در وضعیت {order.LastStatus} قابل تکمیل نیست");
